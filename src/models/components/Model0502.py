...
class Model_w_attention_v3(nn.Module):
    def __init__(self,mode):
        super(Model_w_attention_v3, self).__init__()
        ...
        self.mode=mode
        # 512*3=1536
        self.attnModel = StackedAttention(4,1536, 8)
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x, mask=None, refCov=None):
        ...
        if refCov is not None:
            ...
            ref_weights = F.softmax(refCov, dim=-1)
            # element-wise sum
            #* ref_weights会自动广播到attn_weights的形状2,1,L,L-->2,8,L,L
