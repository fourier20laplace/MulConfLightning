...
import torch
 #* 版本更新 使用repr
class SKEMPIV2Dataset0503(Dataset):
    def __init__(self, data_df, is_train, knn_num, knn_agents_num,rand):
        super(SKEMPIV2Dataset0503, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        if self.rand:
            ...
            repr_wt_tensor=torch.load(os.path.join(wt_file_path,"repr_within_pt_tensor.pt"))
        ...
        if self.rand:
            ...
            repr_mut_tensor=torch.load(os.path.join(mut_file_path,"repr_within_pt_tensor.pt"))
        ...
        batch['mtG']=pt_mt_G
        # 获取cov_tensor
        batch['repr_wt_tensor']=repr_wt_tensor
        batch['repr_mut_tensor']=repr_mut_tensor
        return batch


class SKEMPIV2Dataset0503TST(Dataset):
    def __init__(self, data_df, is_train, knn_num, knn_agents_num,rand):
        super(SKEMPIV2Dataset0503TST, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        # else:
        #     repr_wt_tensor=torch.load(os.path.join(wt_file_path,"repr_within_pt_tensor.pt"))
        if not os.path.exists(os.path.join(wt_file_path,"repr_within_pt_tensor.pt")):
            raise FileNotFoundError(f"File not found: {os.path.join(wt_file_path,'repr_within_pt_tensor.pt')}")
        try:
            repr_wt_tensor=torch.load(os.path.join(wt_file_path,"repr_within_pt_tensor.pt"))
        except Exception as e:
            print(f"Error loading repr_wt_tensor: {e}")
            return 0
        # mt_name=wt_name+'_'+mutate_info.lower()
        # mut_file_path="/home/lmh/projects_dir/Antibody_Mutation/data/SKEMPIv2/PDBs_mutated/af3_sp2_output/"+f"{mt_name}_sp2_rand0.2/gather"
        # PDB_mut_file_path=os.path.join(mut_file_path,"ref.pdb")
        ...
        # else:
        #     repr_mut_tensor=torch.load(os.path.join(mut_file_path,"repr_within_pt_tensor.pt"))
        # if not os.path.exists(os.path.join(mut_file_path,"repr_within_pt_tensor.pt")):
        #     raise FileNotFoundError(f"File not found: {os.path.join(mut_file_path,'repr_within_pt_tensor.pt')}")
        return 1